...
class PianoDataset(BaseDataSet):
    ...
    def _set_files(self):
        file_list = os.path.join(self.root,'segment_'+self.split+'.txt')
        ...
        for item in items:
            ...
            img_line = split_line[0].split('/')
            label_line = split_line[1].split('/')
            self.img_files.append(os.path.join(img_line[-2],img_line[-1]))
            self.label_files.append(os.path.join(label_line[-2],label_line[-1]))
        self.files = self.img_files
    def _load_data(self,index):
        image_id = self.img_files[index]
        lable_id = self.label_files[index]
        image_path = os.path.join(self.root,image_id)
        label_path = os.path.join(self.root,lable_id)
